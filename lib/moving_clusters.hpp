...
// if snowflake, initialize attributes e.g. path, speed etc.
// if cluster, 
// 
//! @brief Main function.
MAIN() {
    ...
    // if (node.uid != 0) {
    //     // init struct?  
    //     times_t t = node.current_time();
    //     vec<3> v = {1, 1, 0};
    //     rectangle_walk(CALL, make_vec(0, 0, 0), make_vec(50, 50, 0), node.storage(tags::speed{}), 1);
    // }

    // define an L-shaped path 
    // std::array<vec<3>, 2> path = {{
    //     {0, side, 0}, 
    //     {0, 0, 0}
    // }};
    // real_t r2 = 10;     // speed at which to follow path
    // real_t period = 1; 
    ...
    // }
    // all other nodes walk randomly within simulation area with a given speed
    // else {
...
//     }
//     // Node has 2 or 3 neighbor nodes, colour it 
//     else if ((4 <= nbr) && (nbr <= 7)) {
...
DECLARE_OPTIONS(list,
	...
	snowflake<0, 50>,
    cluster<devices, 100, 100, 150, 150>,
